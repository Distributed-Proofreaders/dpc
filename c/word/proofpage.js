...
//    divtext         = $("divtext");
    ...
    addEvent(tatext,        "scroll",    eScroll);
...
function applylayout() {
    ...
    tatext.style.lineHeight     = GetLineHeight();
    imgpage.style.width             = (GetZoom() * 10).toString() + 'px';

    boxheight                       = doc.body.clientHeight
                                        - divstatusbar.clientHeight
                                        - (Layout() == "horizontal" ? 4 : 0)
                                        - (IsCtls() ? divcontrols.clientHeight : 0);

    boxwidth                        = doc.body.clientWidth
                                        - (Layout() == "vertical" ? 4 : 0);
...
function eKeyUp() {
    if(tatext.clientHeight != tHeight) {
        divpreview.innerHTML = tatext.value;
        tHeight = tatext.clientHeight;
    }
    consider_wordchecking();
}
...
function maybe_scroll() {
    var p1 = tatext.value;
    var p2 = p1;
    var ipos = tatext.selectionStart;
    p1 = p1.slice(0, ipos) + "<span id='mark_'></span>" + p1.slice(ipos);
    divpreview.innerHTML = p1;
    $('mark_').scrollIntoView();
    tatext.scrollTop = divpreview.scrollTop;
    divpreview.innerHTML = p2;
}
...
function eMouseMove(e) {
    ...
    else if(is_in(tatext, e.clientX, e.clientY)) {
        set_text_id();
    }
    else {
        set_null_id();
...
function eScroll(e) {
    ...
    if(tgt.id == "tatext") {
        if(_active_scroll_id == "tatext") {
            // source is tatext, also scroller, apply to divimage
            apply_scroll_pct();
        }
    }
...
function apply_scroll_pct() {
    ...
    var toctl   = (_active_scroll_id == "tatext"
                    ? divimage
                    : tatext);
 ...
 */
function Hilite(t, y) {
    var t1 = t.substr(0, y[0]);
    var t2 = "<span class='hilite'>" ;
    var t3 = t.substr(y[0]+1, y[1]) ;
    var t4 = "</span>" ;
    var t5 = t.substr(y[0] + y[1] + 1);
...
/*
function eQuoteCheck() {
    var x = [];
    var re = /(\n" | "[\n$]| \?| !)/g;
    var t = tatext.value;
    var q;
    while((q = re.exec(t)) != null) {
        x.push([q.index-1, q[0].length]);
    }
    for(var i = x.length-1; i >= 0; i--) {
        t = Hilite(t, x[i]);
    }
    divpreview.innerHTML = t;
}
*/
...
function GetLineHeight() {
    var strval = getnamevalue("lineheight").toString();
    if(strval == "") {
        strval = "1.45em";
        setnamevalue("lineheight", strval);
    }
    return strval;
...
//    _bwcount = bw_count();
    ...
    $("span_wccount").innerHTML = (wc_count() + bwcount());
