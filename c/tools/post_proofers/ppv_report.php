...
$pper = new DpUser($project->PPer());
$ppver = new DpUser($project->PPVer());
...
// language, so temporarily change the current locale.
	setlocale(LC_ALL, $pper->InterfaceLanguage());
	$ppbita = _("Hello %1\$s,
...
setlocale(LC_ALL, $ppver->InterfaceLanguage());
$ppvbita = _("Hello %1\$s,
...
$ppvbit = sprintf($ppvbita, $ppver->Username(), $project->NameOfWork(), $general_help_email_addr);
...
if ($_POST['cc_pp'])
{
   	$to = $pper->EmailAddress();
   	$subject = "DP PP: {$project->NameOfWork()}";
   	$message = $ppbit.$reportcard.$signoff;
    maybe_mail($to, $subject, $message, $additional_headers);
}

if ($_POST['cc_ppv']) {
   	$to = $ppver->EmailAddress();
   	$subject = "DP PPV: {$project->NameOfWork()}";
...
maybe_mail($to, $subject, $message, "From: {$ppver->Username()} <{$ppver->EmailAddress()}>\r\n");
