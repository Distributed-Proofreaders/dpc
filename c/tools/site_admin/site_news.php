...
if($news_page_id == "") {
    ...
    echo "content";
    echo $content; 
    handle_any_requested_db_updates( $news_page_id, $item_id, $content, $action );

    $news_subject = get_news_subject($news_page_id);
    $title = sprintf('News Desk for %s', $news_subject );
    theme($title, "header");
    echo "<br>";
    echo "<a href='edit_news.php'>"._("Site News Central")."</a><br>";
...
foreach ( $news_pages as $news_page_id => $news_subject ) {
	echo "<!--  $news_page_id   $news_subject   -->\n";
    echo "<li>";

    $news_subject = get_news_subject($news_page_id);
    $link = "<a href='newsedit.php?news_page_id=$news_page_id'>$news_subject</a>";
...
function handle_any_requested_db_updates( $news_page_id, $item_id, $content, $action) {
    global $dpdb;
    ...
    switch( $action ) {
        case 'add':
            ...
            // $content = addslashes($content, '<a><b><i><u><font><img>');
            ...
            assert($dpdb->SqlExecutePS($sql, $args) == 1);
            // by default, new items go at the top
            assert(1 == $dpdb->SqlExecute("
                UPDATE news_items 
                SET ordering = id 
...
function show_item_editor( $news_page_id, $item_id, $action ) {
    ...
    if ($action == "edit") {
        ...
        $submit_button_label = "Save News Item";
    } 
    else {
        $item_id = "";
        $initial_content = "";
    ...
    foreach ( $categories as $category ) {
        ...
        foreach($rows as $news_item) {
            ...
            foreach ( $actions as $action => $label ) {
                $url = "newsedit.php?news_page_id=$news_page_id&amp;item_id={$news_item['id']}&amp;action=$action";
