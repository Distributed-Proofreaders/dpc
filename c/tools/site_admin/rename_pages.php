...
$res = mysql_query("
    SELECT fileid, image FROM $projectid
    ORDER BY image");
    ...
    case 'Check renamings':
        ...
        foreach( array_count_values( $new_fileid_for_ ) as $new_fileid => $freq ) {
            if ( $new_fileid != '' && $freq > 1 ) {
                echo "Error: You have requested $new_fileid as the new fileid 
                        for $freq different pages.\n";
                echo "\n";
                $n_errors++;
            }
        }
        ...
        foreach ( $new_fileid_for_ as $old_fileid => $new_fileid ) {
            echo "<input type='hidden' name='new_fileid_for_[$old_fileid]' 
                value='$new_fileid'>";
        }
        echo "</form>";
        break;
