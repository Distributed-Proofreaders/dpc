...
switch ( $submit_button ) {
    ...
    case 'Do insertions!':
        ...
        echo "\n";
        $url = "$code_url/tools/project_manager/page_detail.php"
                                ."?project={$projectid_['to']}"
                                ."&amp;show_image_size=0";
...
//    $page_names_ = array();
    ...
    foreach ( array( 'from', 'to' ) as $which ) {
        ...
        $res = mysql_query("
            SELECT nameofwork FROM projects
        ...
        if ( $n_projects == 0 ) {
            ...
            die( "projects table has $n_projects matches 
                for projectid='$projectid'. (Can't happen)" );
        }
        ...
        $res = mysql_query("
            SELECT image, fileid FROM $projectid
            ORDER BY image");
        ...
        $res= mysql_query("
            DESCRIBE $projectid") or die(mysql_error());
    ...
    if ( count($column_names_['from']) < count($column_names_['to']) ) {
        ...
        echo "The 'from' project has more columns than the 'to' project, 
            but we can handle this now.\n";
    }
